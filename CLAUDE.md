...
- Parallel sessions share one primary checkout: `orient.sh` lists the live ones and `ListAgents` names them; work in a worktree under `.claude/worktrees/` (`git worktree add … origin/main`, then `EnterWorktree`); never checkout/stash/reset/rebase/merge under another session's work — convention only since 2026-08-30 (the guard that used to deny it, R6, was retired: its liveness check had no time dimension and denied real merges over dead sessions twice in one night); never edit a file another session names as in use. A live peer per `ListAgents` gets a one-line "about to merge PR #N" `SendMessage` ping before `gh pr merge` — async courtesy, not a blocking gate (2026-08-31).
