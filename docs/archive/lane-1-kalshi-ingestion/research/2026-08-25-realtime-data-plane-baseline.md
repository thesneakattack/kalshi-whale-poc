...
**Task:** I0 of `docs/archive/lane-1-kalshi-ingestion/plans/2026-08-25-realtime-data-plane-investigation.md`.
**Date:** 2026-08-25.
...
| H7 | Background REST starves whale-critical calls | **reproduced causally in the model** (I11, `2026-08-25-rest-solution-comparison.md`): with the measured demand shapes and the 8-token local bucket, whale enrichment waits 1.2 s at p95 (4.9 s under a 40-call catalog storm) while upstream never exceeds 150 tokens/s; every policy that keeps critical calls out of the queue behind the tick's gather removes the wait at identical upstream usage. Live: I5/I7 shares and waits match the model; final confirmation is the post-fix I5 metrics | I5/I8 caller-class demand shares + critical-call wait under normal/burst background load | critical-call wait unaffected by background demand at measured volumes |
...
| H9 | Milestone/live-data polling duplicates work | **confirmed, moderate** (I8 follow-up, 2026-08-25): 1,027 milestone-family calls in 24 min vs a 672-call floor for 28 tracked events — duplicate factor 1.53; separately, the lifecycle `settled` `get_market` re-read is 23% of all REST calls (0.55/s vs a documented ~0.06/s estimate) | I8 duplicate-demand measurement showing material overlapping calls for the same milestone/event | negligible measured overlap |
