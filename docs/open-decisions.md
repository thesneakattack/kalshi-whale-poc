...
- `feat/candlestick-volatility` is **parked by recorded decision, not stale**: 5 commits of unmerged candlestick-volatility work, pushed to origin (`ad24098`) so it can't be lost, ~13 ahead / ~800 behind `main` — landing it needs a real rebase/merge pass, not a fast-forward. Decision record: `docs/branch-audit-2026-09-05.md` §2 (the #398 comment it cites is on a closed issue about a different branch; this line is now the canonical record). **Stale-branch triage (AQC at every `/checkpoint`) will keep surfacing it — that signal is correct and stays; the recorded decision only means don't act on it without re-reading this line.** Eventual fate is a real call: rebase-and-land, or discard · you · 2026-09-06
...
- PR #441 follow-ups (Tasks 1–6 turned out already shipped on `main` — the tier0 branch was pushed as PR #618, proven a strict subset, closed, deleted; correction recorded on #448): Tasks 7/9/10 still open; Tier 0 items 3/5/6 are #599 (done)/#609/#527; `market_history.db` corruption watch = weekly `quick_check` in the storage scan · #448 comment (2026-09-06 correction) · me
