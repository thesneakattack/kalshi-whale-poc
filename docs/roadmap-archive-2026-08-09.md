# Roadmap archive (frozen snapshot, 2026-08-09)

**This is a frozen copy, not a living document.** `ROADMAP.md` had grown to
837 lines, almost entirely full-narrative detail on items already shipped —
this file preserves that detail verbatim as of 2026-08-09, so `ROADMAP.md`
itself could be cut down to a short, actually-workable list of what's still
open. Nothing here is maintained going forward: don't check items off in
this file, don't add new ones. For ongoing work see `ROADMAP.md`; for the
continuing phase-by-phase historical record (including everything shipped
after this snapshot was taken) see `static/status.html` (`/status`), which
remains the actively-maintained backward-looking doc.
...
- [x] Per-market-type (per-series) `min_notional_usd` thresholds, instead of
      one global value — new `whale_watcher_kalshi.min_notional_usd_by_series`
      dict, keyed by `signal_log.series_of()` (the same series definition
      `excluded_series`/`series_stats` already use), with the existing
      `min_notional_usd` staying as the fallback default for any series with
      no override. New Config-tab field next to the existing threshold
      field, same comma-separated-pairs convention as `excluded_series`. 3
      new tests. Full suite: 433 (was 430). Side fix found while verifying:
      the `.claude/hooks/run_tests.py` pytest hook was silently broken —
      the repo-root `Dockerfile` only ever installed `requirements.txt`, not
      `requirements-dev.txt` (pytest), so a fresh container build lost the
      ability to run tests entirely. Fixed by baking dev deps into the same
      image; verified via a real `ddev restart`.
