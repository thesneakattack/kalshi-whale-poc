...
Per-module read of the actual `_connect()` body and schema/DDL init path, in full, for `risk_manager.py`, `paper_broker.py`, `candidate_ledger.py`, `series_watcher.py`, `settlement_edge.py`. Every claim below is a direct citation, not inferred. Also includes each module's test call-site shape (`with`-wrapped vs. bare-assignment) per the correction that bare-assignment callers of `_connect()` are a real migration risk the hybrid design must account for (`ce._connect()` in `tools/coordination_engine.py` is called bare-assignment from both its own 22 test call sites AND two real production call sites — `tools/quality_coordination.py:584,657`, zero `.close()` anywhere in that file — so this isn't only a test-fixture risk for that module, it's a genuine live leak in a short-lived CLI process too; recheck correction, not the original framing of this doc).
...
- Test call-site shape: `tests/test_candidate_ledger.py` monkeypatches `DB_PATH` (`:7`) and **does** call `_connect()` directly once, correctly `with`-wrapped: `with candidate_ledger._connect() as conn:` (`:56`) — hybrid-compatible as written, unlike `coordination_engine.py`'s bare-assignment callers (both its tests and its two real production call sites — see the intro note above).
...
- Non-`CREATE TABLE` statements:
  ...
  - **Not the only case of this pattern in the repo** (added during recheck, coordinator cross-reference): `services/candidate_log.py:86,91` executes `capture_writer.REJECTED_CANDIDATES_DDL_SQL` and `capture_writer.REJECTION_EVENTS_DDL_SQL` the same way — two more DDL strings owned by `capture_writer.py`, imported and executed by a different module. `candidate_log.py` isn't one of this doc's 5 named modules, but the same identity-check conflict risk described above applies to it too; the migration plan should treat "modules that import and execute a DDL string owned by `capture_writer.py`" as one category with (at least) two members, not size the fix to `series_watcher.py` alone.
...
2. **Zero of the 5 tests calls `_connect()` in the bare-assignment shape** that `coordination_engine.py` has — every test that does call `_connect()` directly (`candidate_ledger.py`, `series_watcher.py`) already does so `with`-wrapped. The other 3 don't call it at all. None of these 5 modules carries the bare-assignment risk `coordination_engine.py` has (which, per the recheck correction above, is a real production-code leak there too, not only a test-fixture issue) — that risk is real, but it isn't universal, and this pre-audit found no instance of it in this batch.
