...
queue-depth jump in #541/#542's incident-window analysis). `critical_whale.network.window_max_ms`
...
reopens a bug this codebase already paid down once. `self._seen_trade_ids` (mark-seen
ring) and `self._market_cache` (the TTL cache) both look safe under concurrent access
by inspection — pure dict/set reads and writes with no cross-call reset — but were not
exhaustively audited here; that audit is real, scoped work for the eventual
implementation-plan stage, named here so it isn't skipped.
...
other's cache write). The `opened_since=now` stale-price guard pattern
(`strategy_engine.py`, referenced from `_process_stream_ticker`'s own comments as
protection against exactly this class of cross-stream ordering hazard) is the
existing tool for auditing whether any downstream consumer assumes trade-before-ticker
ordering across *different* tickers; not re-derived here, flagged for the
...
`_TokenBucketRateLimiter` (8 req/s, burst 8, `http_client.py:323-325`) across all six
read-side caller classes: `critical_whale`, `critical_position`, `interactive`,
`background_catalog`, `background_live_status`, `background_resolution` (plus
`background_index_backfill`, `other`). Split it — reserve a sub-budget for
`critical_whale` (and arguably `critical_position`, both already named "critical"
and both on request-serving hot paths) separate from the four `background_*`
classes, so a busy `background_catalog` burst can no longer make a `critical_whale`
...
1.4s resolve-call median) — a real, roughly 5-15x improvement to the common case.
...
| **New correctness burden** | Batch-window latency floor (~100-250ms); wider blast radius per failed batch | Must fix `self._resolve_failed_tickers`'s per-call-reset assumption first (found here, not previously known); needs per-ticker concurrency bound | None |
| **Benefit shape** | Counter-cyclical — near-zero help under normal load, most help exactly when backlog is worst (needs B to realize) | Removes resolve latency from the consumer's critical path entirely, regardless of load | Cuts *median* 5-15x; tail (up to 44.6s measured) untouched |
...
- **`trade_stream.ingest.queue_depth` sustained above ~80% of capacity for ≥5
  ...
  exactly the manual falsifier #541/#542's own comment thread used to distinguish
...
incident. **Option A is not a real alternative to B — it is a refinement that only
