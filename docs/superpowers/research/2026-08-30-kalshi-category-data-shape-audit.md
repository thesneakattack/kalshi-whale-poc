...
`widget_status`**, rising to 13.3 % and 28.4 % on the forward-dated slice; every
deviating type but one attaches to a series category the app has configured — the
exception is `kpis`, which resolves to **Companies**, a category the app does not
currently configure. D2 stays at #2. What
...
|---|---|---|---|---|
| no `details.winner` | `esports_match` 9,385 · `one_off_milestone` 420 · `political_race` 36 · `company_report` 337 · `truflation` 241 · `golf_tournament` 169 · `artist_streams` 139 · `kpis` 14 · `tv_views` 1 | **10,742** | 9.5 % | 382+ (13.3 %+) |
...
*itself* a cross-category settlement-input feed, and today's per-type extraction only
looks for `widget_status`/`winner` — an overstatement of "reads exactly one key," since
`game_state.record` (C6) already persists a payload's `raw_json` under sports-shaped
columns for the categories C6 covers, which would make part of D2/D3 cheaper than a
from-scratch integration where it applies, not something requiring a new capture path
outright; whether that persistence path actually reaches `truflation`/`artist_streams`
specifically (Economics/Crypto/Entertainment, not Sports) needs confirming at
implementation time, not assumed here. **Economics/Crypto**: `truflation` milestones (244 population, 241 live) carry
