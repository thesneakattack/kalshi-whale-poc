...
| Path | Lane | Status | Reason |
|---|---|---|---|
| docs/superpowers/specs/2026-08-26-economic-strategy-remediation-design.md | 3 | active | CLAUDE.md's current "Open gaps" line ("pricing/edge gap at entry, 0.60–0.95 unit-cost band negative-EV, designed not implemented") directly matches this design's own proposal; explicitly gated behind Program 1, which has since landed, but no evidence the remediation itself was implemented. |
...
| docs/superpowers/research/2026-09-03-worker-cpu-pin-and-loop-stalls.md | 5 | active | Subject is `loop_watchdog`/uvicorn-worker CPU pin (Lane 5); findings fed the same-window event-loop-blocking fixes that shipped (PR #414/#420/#424 etc., all confirmed merged). |
