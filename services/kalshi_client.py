...
gateway (services/kalshi/public.py) — Phase A Task A6.

This file used to BE the public client; its wire semantics (envelope
handling, SDK/raw-HTTP compatibility, batching) now live in
KalshiPublicGateway, and KalshiClient subclasses it so every existing
caller and test keeps the exact same surface with zero re-implemented
semantics (the design spec's facade rule: "facade methods delegate without
re-implementing semantics" — inheritance is that, provably; see
tests/test_kalshi_public_gateway.py).

What still lives here is market-selection *policy* — which series/markets
are strategically interesting (candidate filtering, series-level
round-robin, watchlist caps). That's an application concern, not a vendor
one, and it migrates into services/market_watch/ at A7. Until then this
facade is its home, unchanged.

Do not add new wire-semantic methods here — they belong on the gateway,
with a CONTRACT_DOCS entry naming their exact docs/kalshi/ sources. New
code should construct/consume the gateway (or the market_watch policy
layer) rather than adding fresh imports of this facade; A15 turns that
into a CI ratchet.
"""
import asyncio
