...
- Evidence first from what exists: `GET /api/health/pipeline`, `GET /api/observability/summary`; the user-built benchmark/probe tools — `tools/realtime_pipeline_replay.py`, `tools/rest_scheduler_replay.py`, `tools/watchlist_scale_stress_test.py`, `tools/kalshi_rate_limit_probe.py`, `tools/kalshi_census.py` — before writing any new measurement script; `observability-performance` skill.
